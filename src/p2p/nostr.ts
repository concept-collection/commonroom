...
// event can arrive several times. Remember recently seen event ids and drop
// repeats so handlers fire exactly once per event.
const SEEN_CAP = 1000
...
/** Topic everyone in a room announces on / listens to for presence. The room
 *  ID is any string (exact match — no normalization). */
export const roomTopic = (roomId: string): Promise<string> =>
  sha256Hex(`commonroom:${roomId}`)
